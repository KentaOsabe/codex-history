...
- サニタイズは SafeList 方針とし、`pre` / `code` / `strong` / `em` / `a` など最小限のタグのみ許可し、`javascript:` スキームやイベント属性は除去する。
...
- API エラーレスポンスは `data: null` と `errors` 配列（`status` は文字列）を持つ共通フォーマットで統一し、ログ出力レベル（4xx=info、5xx=error）を明文化する。
