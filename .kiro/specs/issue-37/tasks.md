...
- [x] 1. VM 変換で My request を本文へ昇格し IDE コンテキストを外す
...
- [x] 2. ユーザーカード UI を本文＋オプションのシンプル構成に改修
  - `MessageCard` の user 分岐で IDE コンテキスト UI をレンダリングしないよう条件分岐し、「本文」→「オプション」順に表示。オプションはデフォルト未選択。
  - オプション未存在時はセクション自体を非表示にする。
  - _Requirements: 1.3, 2.2, 2.3_

- [x] 3. ユーザー本文のタイポをアシスタントと統一
  - `MessageCard.module.css` を調整し、ユーザー本文のフォントサイズ・行間・余白をアシスタントと同等クラスに統一する。
  - 回帰しないよう DOM/CSS snapshot テストを追加または既存テストを更新。
  - _Requirements: 1.2_

- [x] 4. レイアウト比を 7:3 に変更しモバイル挙動を維持
...
- [x] 5. テストと Storybook/Visual 回帰を更新
  - `mapResponseToViewModel.test.tsx`, `MessageCard.userLayout.test.tsx`, `SessionDetailPage.integration.test.tsx` などを追加/更新し、本文位置、オプション初期OFF、レイアウト比、モバイル挙動を検証。
